...
def generate_paired_data(dataset_file, original_key, sampled_key):
    dataset_file = os.path.expanduser(dataset_file)
    loaded = datasets.load_dataset('json', data_files=dataset_file, split='train', cache_dir=cache_dir)

    if original_key not in loaded.column_names:
        raise ValueError(f"Key '{original_key}' not found in {dataset_file}. Available keys: {loaded.column_names}")
    if sampled_key not in loaded.column_names:
        raise ValueError(f"Key '{sampled_key}' not found in {dataset_file}. Available keys: {loaded.column_names}")

    pairs = []
    for original_text, sampled_text in zip(loaded[original_key], loaded[sampled_key]):
        if not isinstance(original_text, str) or not isinstance(sampled_text, str):
            continue
        original_text = strip_newlines(original_text.strip())
        sampled_text = strip_newlines(sampled_text.strip())
        if not original_text or not sampled_text:
            continue
        original_text, sampled_text = trim_to_shorter_length(original_text, sampled_text)
        if not original_text or not sampled_text:
            continue
        pairs.append((original_text, sampled_text))
    ...
    if len(pairs) > 0:
        original_batch = [x[0] for x in pairs]
        sampled_batch = [x[1] for x in pairs]
        tokenized_original = preproc_tokenizer(original_batch)["input_ids"]
        tokenized_sampled = preproc_tokenizer(sampled_batch)["input_ids"]
        pairs = [
            (o, s)
            for (o, s), tok_o, tok_s in zip(pairs, tokenized_original, tokenized_sampled)
            if len(tok_o) <= 512 and len(tok_s) <= 512
        ]
    ...
    print(f"Total number of paired samples: {len(pairs)}")
    if len(pairs) > 0:
        print(f"Average number of words (original): {np.mean([len(x.split()) for x in data['original']]):.2f}")
        print(f"Average number of words (sampled): {np.mean([len(x.split()) for x in data['sampled']]):.2f}")
...
def generate_data(dataset, key):
    # load data
    if args.dataset_file and args.sampled_key:
        return generate_paired_data(args.dataset_file, key, args.sampled_key)
    elif args.dataset_file:
        dataset_file = os.path.expanduser(args.dataset_file)
        loaded = datasets.load_dataset('json', data_files=dataset_file, split='train', cache_dir=cache_dir)
        if key not in loaded.column_names:
            raise ValueError(f"Key '{key}' not found in {dataset_file}. Available keys: {loaded.column_names}")
        data = loaded[key]
    ...
    # keep only non-empty string examples, then strip whitespace
    data = [x.strip() for x in data if isinstance(x, str) and x.strip()]

    # remove newlines from each example
    data = [strip_newlines(x) for x in data]

    # try to keep only examples with > 250 words for built-in long-form datasets
    if (not args.dataset_file) and dataset in ['writing', 'squad', 'xsum']:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_file', type=str, default=None,
                        help="Optional local JSON/JSONL file path. When set, dataset is loaded from this file and dataset_key selects the text column.")
    parser.add_argument('--sampled_key', type=str, default=None,
                        help="Optional second text key from dataset_file. When set, uses dataset_key as original and sampled_key as machine-generated samples.")
    ...
    parser.add_argument('--cache_dir', type=str, default="/app/.hf_cache")
    args = parser.parse_args()

    if args.sampled_key and not args.dataset_file:
        raise ValueError("--sampled_key requires --dataset_file")
